...
static Link insert (Link t, Item it)
{
	...
	/*if(!eq(t->value, it)){
		if(less(it, t->value))
			t->left->within->ncompares = t->within->ncompares + 1;
		else
			t->right->within->ncompares = t->within->ncompares + 1;
	}*/
	//printf("%d %d\n", t->value, t->within->ncompares);
	return t;
}
...
static Link insertAtRoot (Link t, Item it)
{
	...
	//if(diff < 0)
	//	t->within->ncompares = t->right->within->ncompares + 1;
	//else if(diff > 0)
	//	t->within->ncompares = t->left->within->ncompares + 1;
	//printf("%d %d\n", t->value, t->within->ncompares);
	return t;
}
