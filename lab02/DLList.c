...
void DLListBefore (DLList L, char *it)
{
	...
	struct DLListNode* newItem = newDLListNode(it);
	if(L->first == NULL){
	  L->first = L->last = L->curr = newItem;
	  return;
	}
	if(L->curr == L->first)
	  L->first = newItem;
	else{
	  L->curr->prev->next = newItem;
	  newItem->prev = L->curr->prev;
	}
	newItem->next = L->curr;
	L->curr->prev = newItem;
	L->curr = L->curr->prev;
...
void DLListAfter (DLList L, char *it)
{
	...
	if(L->curr == L->first)
	  ...
	  L->curr->next->prev = newItem;
	  newItem->next = L->curr->next;
	}
	L->curr->next = newItem;
	newItem->prev = L->curr;
	L->curr = L->curr->next;
...
void DLListDelete (DLList L)
{
	assert (L != NULL);
	/// COMPLETE THIS FUNCTION
	struct DLListNode* target = L->curr;
	if(L->curr == L->last){
	  L->curr = L->curr->prev;
	  L->last->prev = NULL;
	  L->curr->next = NULL;
