...
int connections (Map g, LocationID start, LocationID end, TransportID type[])
{
	...
	int idx = 0;
	for (VList currLocation = g->connections[start]; currLocation != NULL; currLocation = currLocation->next) {
		if (currLocation->v == end)
			type[idx++] = currLocation->type;
		/*
		else if (currLocation->type == BOAT) {
			for (VList currViaSea = currLocation; currViaSea != NULL; currViaSea = currViaSea->next) {
				if (currViaSea->v == end && (isLand(start) && isLand(end))) {
					type[idx++] = BOAT;
					printf("here\n");
					break;
				}
		...
		*/
		if (isSea(currLocation->v)) {
			LocationID target = currLocation->v;
			for (VList curr = g->connections[end]; curr != NULL; curr = curr->next) {
				if (curr->v == target)
					type[idx++] = BOAT;
			}
		}
	...
	return idx;  // to keep the compiler happy
}
